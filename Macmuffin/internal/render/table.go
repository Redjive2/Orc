// Package render draws Macmuffin's output: the board and the card.
//
...
// and every width is clamped to a sane minimum so a degenerate pool — no tasks,
// no scope, a one-character name — still produces a well-formed table.
//
...
func (c Cell) render(p style.Palette, width int, align Align) (string, error) {
	...
	text := theme.Sanitise(c.Text)
	padded, err := theme.Pad(text, width, byte(align))
	if err != nil {
		return "", err
	}
...
type Table struct {
	...
	Note string
	// NotePaint styles the note. Nil leaves it muted, which is right for a
	// count and wrong for a verdict.
	NotePaint func(style.Palette, string) string
...
func measure(t Table, width int) ([]int, error) {
	...
	for i, c := range t.Columns {
		widths[i] = maxInt(theme.Width(c.Header), c.Min, 1)
	}
	for _, row := range t.Rows {
...
func widen(t Table, widths []int, width int) {
	need := theme.Width(theme.Sanitise(t.Title)) + theme.Width(theme.Sanitise(t.Note)) + 3
	if empty := theme.Width(theme.Sanitise(t.Empty)) + 2; len(t.Rows) == 0 && empty > need {
		need = empty
	}
...
func drawTitle(b *strings.Builder, t Table, p style.Palette, inner int) error {
	...
	title := theme.Sanitise(t.Title)
	note := theme.Sanitise(t.Note)
	...
	noteWidth := minInt(theme.Width(note), room/2)
	titleWidth := room - noteWidth
	// A title long enough to fill its share is truncated flush against the
	// note, which reads as one run-on string. One column of the title's space
	// belongs to the gap.
	gap := 0
	if noteWidth > 0 && titleWidth > 1 {
		titleWidth--
		gap = 1
	}
	if titleWidth < 1 {
	...
	left, err := theme.Pad(title, titleWidth, 'l')
	if err != nil {
		return err
	}
	right, err := theme.Pad(note, noteWidth, 'r')
	if err != nil {
		return err
	}
	...
	notePaint := p.Muted
	if t.NotePaint != nil {
		notePaint = func(s string) string { return t.NotePaint(p, s) }
	}
	b.WriteString(paint(p, notePaint, right))
	b.WriteString(" ")
	b.WriteString(p.Frame(vertical))
	b.WriteString("\n")
	return nil
...
func drawEmpty(b *strings.Builder, t Table, p style.Palette, inner int) error {
	...
	padded, err := theme.Pad(theme.Sanitise(text), inner-2, 'c')
	if err != nil {
		return err
	}
	b.WriteString(p.Frame(vertical))
