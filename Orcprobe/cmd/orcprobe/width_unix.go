...
// deliberately — and it is also how a test or a script pins the width without a
// terminal at all. Otherwise the ioctl answers, and any failure returns zero so
// the renderer falls back to its own default.
//
// The ioctl is raw rather than golang.org/x/sys, because Orcprobe is stdlib
// only, as Mailman and Anno are.
