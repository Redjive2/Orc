// Package render draws Orcprobe's output.
//
// Rendering is two passes, as in Anno and Mailman: a layout pass measures every
// cell, then a draw pass emits fixed-width rows. Both are pure functions of
// their input, and every width is clamped, so a degenerate probe — no sources,
// an empty name, one identity — still produces a well-formed table.
//
// Colour is a layer and never information, which is what lets the golden tests
// run with colour off and still pin the whole layout.
package render
...
func (c Cell) render(p style.Palette, width int, align Align) (string, error) {
	// Sanitised first: much of what orcprobe draws came out of a store some
	// other process wrote, and an escape sequence in one of those would repaint
	// every row below it.
	...
	// A cell with nothing in it is left exactly as it is.
	//
	// Without this the lead and the gap below both count the same spaces: a
	// one-space centred cell — the header of the unread-marker column — comes
	// back two spaces wide, and every column after it shifts by one. Colour
	// would then be changing the layout, which is the one thing it must never
	// do. The strip-equivalence test in internal/cli is what caught it.
...
// widen grows the table so its title bar is not clipped. A one-column list of
// probe names under the title "probes" is the ordinary case, and truncating the
// title there would be absurd.
