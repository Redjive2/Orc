...
// CheckTimeout bounds the whole inspection.
//
...
func (o Options) Check(ctx context.Context) Status {
	...
	got.Head = o.git(ctx, source, "rev-parse", "--short", "HEAD")
	got.Branch = o.git(ctx, source, "rev-parse", "--abbrev-ref", "HEAD")
	got.Detached = got.Branch == "" || got.Branch == "HEAD"
	got.Upstream = o.git(ctx, source, "rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}")
	...
	if why := o.checkUpstream(ctx, source); why != "" {
		...
		got.Reasons = append(got.Reasons, Reason{Level: Stop, Text: text, Fix: fix})
	}
	...
	if got.Upstream != "" {
		...
		fields := strings.Fields(o.git(ctx, source, "rev-list", "--left-right", "--count", "@{u}...HEAD"))
		if len(fields) == 2 {
	...
	_, err := o.run(ctx, source, "go", "version")
	got.Toolchain = err == nil
	_, err = os.Stat(filepath.Join(source, "sh", "build"))
	got.Script = err == nil
...
func (o Options) judge(got Status) []Reason {
	...
	if got.Dirty > 0 {
		...
		out = append(out, Reason{
			Level: map[bool]string{true: Caution, false: Stop}[o.Dirty],
