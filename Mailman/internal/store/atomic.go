...
// Permissions. The store holds credentials and private mail, so nothing in it
// is readable by anyone but its owner. This is the boundary the whole security
// model rests on, which is why it is stated once, here, and applied everywhere.
const (
...
//
// Messages and receipts are write-once, and "write-once" has to be enforced
// rather than assumed: two agents replying at the same instant could otherwise
// have one silently overwrite the other. The existence check and the rename are
// not atomic together, so the check is a courtesy that produces a good error;
// the identifier's randomness is what actually makes a collision impossible.
...
// a property to build a mailbox on.
